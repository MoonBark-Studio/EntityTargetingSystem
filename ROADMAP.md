# EntityTargetingSystem — Roadmap

**Module:** `internal.EntityTargetingSystem`

## Action Items from Latest Audit
- [ ] ECS refactor: move 4 files with ECS types into ECS/ subdirectory
  - `plugins/EntityTargetingSystem/Core/AbilityTargetingComponent.cs`
  - `plugins/EntityTargetingSystem/Core/BaseTargetingValidator.cs`
  - `plugins/EntityTargetingSystem/Core/ITargetingValidator.cs`
## TODO
<!-- Action items in priority order -->

## Future Work
<!-- Action items in priority order -->

## Changelog
### 2026-04-18
- Audit run — 0 total issues (0 changed files)
- ECS violations: 4
- Framework bridge gaps: 0

### v0.0.1
- Initial trio created by golden_trio_cron v2

### 2026-04-18
- No auto-fixable issues found.
## Future Work
<!-- Action items in priority order -->

## Changelog
### 2026-05-05
- Audit run — 0 total issues (12 changed files)
- ECS violations: 4
- Framework bridge gaps: 0

<!-- previous entries preserved -->
## Future Work
<!-- Action items in priority order -->

## Changelog
### 2026-04-30
- Audit run — 0 total issues (0 changed files)
- ECS violations: 4
- Framework bridge gaps: 0

<!-- previous entries preserved -->
## Future Work
<!-- Ideas, nice-to-haves -->

## Changelog
### 2026-04-30
- Audit run — 0 total issues (0 changed files)
- ECS violations: 4
- Framework bridge gaps: 0

<!-- previous entries preserved -->